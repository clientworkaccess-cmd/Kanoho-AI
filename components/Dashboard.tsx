...
const Dashboard: React.FC = () => {
  ...
  return (
    <div className="flex h-screen bg-[#050505] overflow-hidden relative">
       {/* Sleek Sidebar */}
       <aside className="w-20 lg:w-72 premium-glass border-r border-white/5 flex flex-col justify-between shrink-0 z-30">
        <div>
          <div className="h-20 flex items-center justify-center lg:justify-start lg:px-8 border-b border-white/5 bg-white/[0.01]">
             ...
             <div className="hidden lg:block ml-4">
                <span className="block font-display font-bold tracking-tight text-white text-lg">KANOHO</span>
                <span className="block text-[10px] text-indigo-400 tracking-[0.2em] font-mono uppercase">Artificial Intelligence</span>
             </div>
          </div>
          ...
          <nav className="mt-10 flex flex-col gap-2 px-3 lg:px-6">
            {navItems.map((item) => (
              ...
              >
                ...
                <item.icon className={`w-5 h-5 shrink-0 transition-colors ${activeNav === item.id ? 'text-indigo-400' : 'group-hover:text-white'}`} />
        ...
        <div className="p-6 border-t border-white/5 bg-white/[0.01]">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div className="hidden lg:block overflow-hidden">
              <p className="text-sm font-semibold text-white truncate font-display">{profile?.full_name || 'Agent'}</p>
              <p className="text-[10px] text-zinc-500 truncate font-mono uppercase tracking-wider">{profile?.email}</p>
            </div>
          </div>
          <button 
            onClick={signOut}
            className="flex items-center gap-3 text-rose-500/80 hover:text-rose-400 transition-colors w-full justify-center lg:justify-start text-xs font-medium uppercase tracking-widest px-2"
          >
      ...
      <main className="flex-1 flex flex-col h-full relative overflow-hidden">
        {/* Modern Header */}
        <header className="h-20 premium-glass border-b border-white/5 flex items-center justify-between px-8 z-20 bg-transparent">
